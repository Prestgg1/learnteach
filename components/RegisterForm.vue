<script setup>
import { useRegisterStore } from "../store/Registration";
import { useApiFetch } from "../composables/useApiFetch"; // Əgər export etməmisənsə, bunu əlavə et.
...
const onFormSubmit = async ({ valid }) => {
    if (valid) {
        // API sorğusunu fetch et
        const { data, error } = await useApiFetch("auth/register/step1", {
            method: "POST",
            headers: {
        ...
        useRegisterStore().registerStep1(
            initialValues.email,
            initialValues.password,
        );
        useRegisterStore().steping();
        toast.add({
            severity: "success",
